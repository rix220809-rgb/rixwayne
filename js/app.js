...
function cleanupOldServiceWorkers(){
  if (window.caches) {
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k.includes('our-memories') && k !== 'our-memories-v10.5.2').map(k => caches.delete(k))))
      .catch(()=>{});
  }
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js?v=10.5.2').catch(err=>console.warn('SW register failed', err));
  }
}
