...
function taipeiHour(date = new Date()) {
  return Number(new Intl.DateTimeFormat("en-GB", { timeZone: TIME_ZONE, hour: "2-digit", hour12: false }).format(date));
}
...
async function loadLatestPillStart(
  ...
  if (explicit.error && explicit.error.code !== "PGRST205" && explicit.error.code !== "PGRST116") {
    throw explicit.error;
  }

  const explicitStart = explicit.data?.start_date ? String(explicit.data.start_date) : null;
  const scheduledStart = activeCycle ? addDays(activeCycle.start_date, 4) : null;
  const belongsToCurrentPeriod = !!(explicitStart && activeCycle && explicitStart >= activeCycle.start_date);

  if (activeCycle && !belongsToCurrentPeriod && scheduledStart) {
    if (activeDay !== null && activeDay < 5) {
      return { startDate: scheduledStart, source: "period_pending" };
    }
    if (activeDay !== null && activeDay >= 5) {
      const { error } = await supabase.from("pill_cycles").upsert({
        space_id: SPACE_ID,
        start_date: scheduledStart,
        note: "由經期 Day 5 自動建立",
      }, { onConflict: "space_id,start_date" });
      if (error) throw error;
      return { startDate: scheduledStart, source: "period_auto" };
    }
  }

  return { startDate: explicitStart, source: explicitStart ? "pill_cycles" : "none" };
}
...
Deno.serve(async (req) => {
  try {
    ...
    const requestedRound = payload.round === "second" ? "second" : payload.round === "first" ? "first" : null;
    const dailyQuestionRound = requestedRound || (taipeiHour() >= 22 ? "second" : "first");
    ...
    if (mode === "pill") {
      const loadedPill = await loadLatestPillStart(supabase, today, activeCycle, activeDay);
    ...
    if (mode === "daily_question") {
      ...
      for (const owner of OWNERS) {
        if (!answered.has(owner)) {
          notices.push({
            owner,
            type: dailyQuestionRound === "second" ? "daily_question_2230" : "daily_question_2030",
            title: dailyQuestionRound === "second" ? "🌙 今日必答最後提醒" : "❤️ 今日必答尚未完成",
            body: answered.size
              ? (dailyQuestionRound === "second" ? "對方已經答完了，睡前只差你這一題。" : "對方已經回答了，現在只差你。")
              : (dailyQuestionRound === "second" ? "今天兩個人都還沒完成默契題，睡前記得來回答。" : "今天的默契題還沒回答，記得在睡前完成。"),
            target: "game"
          });
        }
      }
    ...
    if (!notices.length) {
      return response({ ok:true, mode, sent:0, today, cycleSource, activeDay, latestPeriodDay, pillStartDate, pillDay, pillSource, predictedStart, reason:"今天不符合通知條件", dailyQuestionRound: mode === "daily_question" ? dailyQuestionRound : null });
    }
    ...
    return response({
      ...
      pillSource,
      predictedStart,
      dailyQuestionRound: mode === "daily_question" ? dailyQuestionRound : null,
      results
    });
  } catch (error) {
    console.error(error);
